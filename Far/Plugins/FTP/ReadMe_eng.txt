...
~~~~~~
  ...
  Here is description for full files list:
    ...
    - CopyDlg_*.reg    + Registry files contains design for copy progress
                         dialog designed by different person;
...
correct version of plugin package;
  ...
  You should overwrite next files if they contains in package:
      - *.dll - FTP client or plugins files;
                Client may not run or may fail if you will use it with
                sub-plugins from different versions;
